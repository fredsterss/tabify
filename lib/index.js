 ...
 */
function Tabify (linksEl, targetsEl) {
  if (!(this instanceof Tabify)) return new Tabify(linksEl, targetsEl);
...
Tabify.prototype.hide = function (tab) {
  classes(tab.link).remove('active');
  classes(tab.target).add('hide');
  return this;
}
...
Tabify.prototype.show = function (tab) {
  this.hideAll();
  classes(tab.link).add('active');
  classes(tab.target).remove('hide');
  return this;
}
